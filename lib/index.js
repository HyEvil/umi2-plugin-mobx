...
function getStoresWithRoutes(routes, api) {
  ...
  return routes.reduce((memo, route) => [...memo, ...(route.component && route.component.indexOf('() =>') !== 0 ? getPageStores((0, _path.join)(paths.cwd, route.component), api) : []), ...(route.routes ? getStoresWithRoutes(route.routes, api) : [])], []);
}
...
function isPagesPath(path, api) {
  const paths = api.paths,
        winPath = api.winPath;
  return isDirPath(path) && (0, _utils.endWithSlash)(winPath(path)) === (0, _utils.endWithSlash)(winPath(paths.absPagesPath));
}
...
function _default(api, opts = {}) {
  ...
  function getGlobalStoresContent() {
    return getGlobalStoresFiles().map(({
      name,
      path
    }) => `"${name}": (require('${path}').default) || {},`.trim()).join('\r\n');
  }
  ...
  if (shouldImportDynamic) {
    api.addRouterImport({
      source: 'umi-plugin-mobx/dynamic',
  ...
  api.addVersionInfo([`mobx@${require((0, _path.join)(mobxDir, 'package.json')).version} (${mobxDir})`, `mobx-react@${require('mobx-react/package').version}`]);
